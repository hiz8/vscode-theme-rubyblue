# Ruby Blue Theme

Dark, high contrast theme for Visual Studio Code.

## Install

1. Open VS Code
2. Press `Ctrl+Shift+X` (or `Cmd+Shift+X` on macOS) to open the Extensions view
3. Search for `RubyBlue Theme`
4. Click Install
5. Press `Ctrl+K Ctrl+T` (or `Cmd+K Cmd+T` on macOS) to open the theme picker
6. Select "Ruby Blue" from the list

Alternatively, you can install it from the command palette (`Ctrl+Shift+P` or `Cmd+Shift+P`):

- Type `Extensions: Install Extensions`
- Search for `RubyBlue Theme`
- Click Install

## Screenshots

### HTML

![html screenshot](https://raw.githubusercontent.com/hirofumii/vscode-theme-rubyblue/images/html_01.png)

### CSS

![css screenshot](https://raw.githubusercontent.com/hirofumii/vscode-theme-rubyblue/images/css_01.png)

### JavaScript

![javascript screenshot](https://raw.githubusercontent.com/hirofumii/vscode-theme-rubyblue/images/js_01.png)

## Release Notes

### Version 3.0.0

- **Major refactoring to modern VS Code theme standards**
- Updated minimum VS Code version to ^1.80.0
- Renamed theme file to follow modern naming convention (-color-theme.json suffix)
- Added comprehensive UI color tokens
- Added semantic highlighting support with semanticTokenColors
- Enhanced support for:
  - Activity Bar, Side Bar, Status Bar, Title Bar
  - Peek view, breadcrumbs, and editor widgets
  - Symbol icons and problem indicators
- Updated workspace configuration to modern standards
- Improved accessibility and high contrast support

### Version 2.1.0

- Update theme to fix indent guide being purple.
- Update icon.

### Version 2.0.0

- Updated to look more like the brackets version of the theme.
- Changed theme to JSON format
- Added MarkDown Formatting

### Version 1.0.0

- Initial release

![flogs](https://raw.githubusercontent.com/hirofumii/vscode-theme-rubyblue/images/flogs.png)
**Happy hacking!**
